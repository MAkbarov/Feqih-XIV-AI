...
function BrandingTabs({ settings }) {
    ...
    // Selected files for each variant (not yet uploaded)
    ...
    // Select logo file for current variant
    const selectLogo = (file) => {
        ...
        if (!allowedTypes.includes(file.type)) {
            toast.error('Yalnız JPG, PNG, WebP və GIF formatları dəstəklənir');
            return;
        }
        
        // Validate file size (1MB)
        if (file.size > 1024 * 1024) {
            toast.error('Fayl ölçüsü 1MB-dan çox olmamalıdır');
            return;
        }
        
        const previewUrl = createPreview(file);
        setSelectedFiles(prev => ({ ...prev, [currentKey]: file }));
        setPreviews(prev => ({ ...prev, [currentKey]: previewUrl }));
        toast.success(`${device} ${mode} logo seçildi`);
    };

    // Select favicon file
    const selectFavicon = (file) => {
        if (!file) return;
        
        // Validate file type
        const allowedTypes = ['image/png', 'image/jpeg', 'image/jpg', 'image/x-icon', 'image/vnd.microsoft.icon'];
        if (!allowedTypes.includes(file.type)) {
            toast.error('Yalnız PNG, JPG və ICO formatları dəstəklənir');
            return;
        }
        
        // Validate file size (512KB)
        if (file.size > 512 * 1024) {
            toast.error('Favicon ölçüsü 512KB-dan çox olmamalıdır');
            return;
        }
        
        const previewUrl = createPreview(file);
        setSelectedFiles(prev => ({ ...prev, favicon: file }));
        setPreviews(prev => ({ ...prev, favicon: previewUrl }));
        toast.success('Favicon seçildi');
    };
    ...
    const uploadAllFiles = async () => {
        ...
        try {
            ...
            for (const variant of ['desktop_light', 'desktop_dark', 'mobile_light', 'mobile_dark']) {
                if (selectedFiles[variant]) {
                    const form = new FormData();
                    form.append('image', selectedFiles[variant]);
                    form.append('variant', variant);
                    
                    try {
                        const res = await fetch('/admin/upload/site-logo', { 
                            method: 'POST', 
                            body: form, 
                            headers: { 
                        ...
                        if (!js.success) {
                            errors.push(`${variant}: ${js.message}`);
                        }
                    } catch (e) {
                        errors.push(`${variant}: Yükləmə xətası`);
                    }
                }
            }
            ...
            if (selectedFiles.favicon) {
                const form = new FormData();
                form.append('image', selectedFiles.favicon);
                ...
                try {
                    ...
                    const js = await res.json();
                    if (!js.success) {
                        errors.push(`Favicon: ${js.message}`);
                    }
                } catch (e) {
                    errors.push('Favicon: Yükləmə xətası');
                }
            }
            
            if (errors.length === 0) {
                toast.success('Bütün fayllar uğurla yükləndi!');
                ...
                toast.error('Bəzi fayllar yüklənmədi: ' + errors.join(', '));
            }
        } catch (e) {
            toast.error('Ümumi yükləmə xətası');
        } finally {
        ...
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="backdrop-blur bg-white/80 dark:bg-gray-800/80 border border-gray-200 dark:border-gray-600 rounded-xl p-4">
                    ...
                    >
                        <input id={`logo-file-${currentKey}`} type="file" accept="image/*" className="hidden" onChange={e => selectLogo(e.target.files?.[0])} />
                        ...
                        {previews[currentKey] && (
                            <div className="mt-3 flex items-center justify-center">
                                <div className="relative">
                                    <img src={previews[currentKey]} alt="Logo Preview" className="h-16 w-16 object-contain rounded-lg border" />
                                    {selectedFiles[currentKey] && (
                ...
                <div className="backdrop-blur bg-white/80 dark:bg-gray-800/80 border border-gray-200 dark:border-gray-600 rounded-xl p-4">
                    ...
                    <div
                        ...
                        onDrop={(e) => {
                            ...
                            e.stopPropagation();
                            const file = e.dataTransfer.files?.[0];
                            if (file) selectFavicon(file);
                        }}
                        onClick={() => document.getElementById(`favicon-file` )?.click()}
                    >
                        <input id={`favicon-file`} type="file" accept="image/png,image/jpeg,image/x-icon,image/vnd.microsoft.icon,.ico" className="hidden" onChange={e => selectFavicon(e.target.files?.[0])} />
                        <div className="text-sm text-gray-600 dark:text-gray-300">
                            Şəkli buraya atın və ya <span className="text-emerald-600 dark:text-emerald-400 font-medium">seçin</span>
                        </div>
                        ...
                        {previews.favicon && (
                            <div className="mt-3 flex items-center justify-center gap-2">
                                <div className="relative">
                                    <img src={previews.favicon} alt="Favicon Preview" className="h-6 w-6 object-contain" />
                                    {selectedFiles.favicon && (
                                        <div className="absolute -top-1 -right-1 bg-green-500 text-white rounded-full w-3 h-3 flex items-center justify-center text-xs">
                                            ✓
                                        </div>
                                    )}
                                </div>
                                <div className="text-xs text-gray-500 dark:text-gray-400 truncate max-w-[180px]">
                                    {selectedFiles.favicon ? selectedFiles.favicon.name : previews.favicon}
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-700">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium text-blue-700 dark:text-blue-300">
                                {selectedCount} fayl seçildi və yükləməyə hazırdır
                            </span>
                        </div>
                        <button
                            type="button"
                            ...
                            className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white rounded-lg font-medium transition-colors flex items-center gap-2"
                        >
                            {isUploading ? (
                                <>
                    ...
                    <div className="mt-3 text-xs text-blue-600 dark:text-blue-400">
                        ...
                        <div className="flex flex-wrap gap-2 mt-1">
                            {selectedFiles.desktop_light && <span className="px-2 py-1 bg-blue-100 dark:bg-blue-800 rounded">Masaüstü Aydın</span>}
                            {selectedFiles.desktop_dark && <span className="px-2 py-1 bg-blue-100 dark:bg-blue-800 rounded">Masaüstü Tünd</span>}
                            {selectedFiles.mobile_light && <span className="px-2 py-1 bg-blue-100 dark:bg-blue-800 rounded">Mobil Aydın</span>}
                            {selectedFiles.mobile_dark && <span className="px-2 py-1 bg-blue-100 dark:bg-blue-800 rounded">Mobil Tünd</span>}
                            {selectedFiles.favicon && <span className="px-2 py-1 bg-blue-100 dark:bg-blue-800 rounded">Favicon</span>}
                        </div>
                    </div>
                </div>
            )}
