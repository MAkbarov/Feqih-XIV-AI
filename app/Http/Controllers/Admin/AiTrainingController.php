...
class AiTrainingController extends Controller
{
    ...
    public function __construct(TrainingService $trainingService)
    {
        ...
        // Enhanced service-i manual olaraq yaradırıq
    ...
    public function importFromUrl(Request $request)
    {
        $validated = $request->validate([
            ...
            'crawl_sibling' => 'boolean',  // 🆕 Sibling mod
            'category' => 'nullable|string|max:50',
            'source' => 'nullable|string|max:100',
        ]);

        $single = $validated['single'] ?? false;  // DÜZƏLTMƏ: default false olmalıdır
        ...
        try {
            ...
            // Metod 2 üçün köhnə service istifadə et (işləyir)
            // Enhanced service link extraction problemi var
            if (!$single && $maxDepth > 1) {
                // Bütün sayt üçün köhnə service (daha stabil)
            ...
            if ($result['success']) {
                ...
                // ✅ RAG aktivdirsə, yaradılan/yeni-lənən bütün məlumatlar üçün indeksləmə job-u işə sal
                try {
    ...
    public function reindexKnowledge($id)
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'İndeksləmə başladı! Status yoxlanılsın.'
            ]);
            
        } catch (\Exception $e) {
            \Log::error('Reindex xətası', [
                'kb_id' => $id,
                'error' => $e->getMessage()
            ]);
    ...
    public function deleteAllKnowledge()
    {
        ...
        try {
            ...
            if ($totalCount === 0) {
                return redirect()->back()->with('error', 'Bilik bazasında siləcək məlumat yoxdur.');
            }
            
            // FK-lər üçün təhlükəsiz təmizləmə: əvvəl child, sonra parent
            \DB::beginTransaction();
            try {
                // FK check-ləri söndürüb truncate et (daha sürətli)
            ...
            // Həmişə redirect qaytar (Inertia üçün)
            ...
            return redirect()->back()->with('error', 'Bilik bazasını təmizləyərkən xəta baş verdi: ' . $e->getMessage());
        }
    }
    ...
    public function importKnowledge(Request $request)
    {
        ...
        if ($extension !== 'json') {
            return redirect()->back()->with('error', 'Yalnız JSON formatlı fayllar qəbul edilir.');
        }
